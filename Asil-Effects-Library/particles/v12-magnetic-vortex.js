    ...
    class Particle {
        ...
        update() {
            if (targets.length > 0) {
                ...
                // IDLE MODE: MAGNETIC VORTEX FIELD
                ...
                // If too close to center, reset to edge to keep flow continuous
                if (dist < 50) {
                    this.init();
                    // Place at edge
                ...
                // 2. Add Orbit (Perpendicular) force: + PI/2
                // Mix of Spiral In (Attract) and Orbit
                // angle += 1.2; // Defines spiral tightness

                // 3. Add Simplex Noise for Organic Variation (Wiggle)
                // This prevents perfect sterile circles
                const noise = Simplex.noise(this.x * 0.002, this.y * 0.002 + time * 0.0001);
                angle += noise * 0.5; // Distort field by noise

                // 4. Velocity Vector
                // Speed increases slightly as they get closer (Gravity-like) but cap it
                let speed = (this.speedFactor || 1) * (2 + 1000 / (dist + 100));
                ...
                // Mouse Repel (Stronger)
                let mdx = mouse.x - this.x, mdy = mouse.y - this.y, md = Math.sqrt(mdx * mdx + mdy * mdy);
                if (md < mouse.radius) {
                    let f = (mouse.radius - md) / mouse.radius;
                    let mang = Math.atan2(mdy, mdx);
                    this.vx -= Math.cos(mang) * f * 2;
                    this.vy -= Math.sin(mang) * f * 2;
                }
            }
        }
        draw() {
